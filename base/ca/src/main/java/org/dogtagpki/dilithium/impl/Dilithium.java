...
import org.dogtagpki.dilithium.interfaces.DilithiumPrivateKey;
...
public class Dilithium {
	...
	public static KeyPair generateKeyPair(DilithiumParameterSpec spec, byte[] seed) {
		...
		byte[] o = Utils.getSHAKE256Digest(128, zeta);
		byte[] rho = new byte[32];
		byte[] sigma = new byte[64];
		byte[] K = new byte[32];

		System.arraycopy(o, 0, rho, 0, 32);
		System.arraycopy(o, 32, sigma, 0, 64);
	...
	public static byte[] sign(DilithiumPrivateKey prv, byte[] M) {
		...
		conc = Utils.concat(prv.getK(), Utils.concat(seed, mu));
